...
def _run_meta_analysis(ctx: PaperContext):
    """元分析：排放因子法 vs 实测法 vs 模型法"""
    from meta_analysis_module import MetaAnalysisAgent
    agent = MetaAnalysisAgent(
        data_path=ctx.data_path,
        output_dir=os.path.join(ctx.output_dir, 'meta_analysis'),
    )
    results = agent.run(df=ctx.df)
    ctx.meta_findings = results['findings']
    ctx.meta_group_stats = results['group_stats']
    ctx.meta_test_results = results['test_results']
    ctx.meta_figures = results['figures']
    # 合并到主 findings
    if hasattr(ctx, 'findings') and ctx.findings:
        ctx.findings.extend(results['findings'])
    else:
        ctx.findings = results['findings']
    logger.info(f"元分析完成: {len(results['findings'])} 条发现")
    return results['findings']


def _run_dispersion_analysis(ctx: PaperContext):
    """离散分析：各方法内部变异特征"""
    from dispersion_analysis_module import DispersionAnalysisAgent
    agent = DispersionAnalysisAgent(
        data_path=ctx.data_path,
        output_dir=os.path.join(ctx.output_dir, 'dispersion'),
    )
    results = agent.run(df=ctx.df)
    ctx.dispersion_findings = results['findings']
    ctx.dispersion_stats = results['dispersion_stats']
    ctx.decomp_results = results['decomp_results']
    ctx.dispersion_figures = results['figures']
    # 合并到主 findings
    if hasattr(ctx, 'findings') and ctx.findings:
        ctx.findings.extend(results['findings'])
    else:
        ctx.findings = results['findings']
    logger.info(f"离散分析完成: {len(results['findings'])} 条发现")
    return results['findings']
...
MODULE_REGISTRY = {
    ...
    'meta_analysis': {
        'needs': ['df'],
        'provides': ['meta_findings'],
        'run': _run_meta_analysis,
        'description': '元分析：排放因子法 vs 实测法 vs 模型法',
    },
    'dispersion_analysis': {
        'needs': ['df'],
        'provides': ['dispersion_findings'],
        'run': _run_dispersion_analysis,
        'description': '离散分析：各方法内部变异特征',
    },
    'literature_recall': {
